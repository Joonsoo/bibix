...
data class ClassPkg(
  ...
  companion object {
    fun fromBibix(value: BibixValue): ClassPkg {
      ...
      val deps = (body.pairs[2].second as SetValue).values.map { fromBibix(it) }
...
sealed class CpInfo {
  ...
  companion object {
    fun fromBibix(value: BibixValue): CpInfo {
      value as DataClassInstanceValue
      value.value as DataClassInstanceValue
      return when ((value.value as DataClassInstanceValue).className.tokens) {
