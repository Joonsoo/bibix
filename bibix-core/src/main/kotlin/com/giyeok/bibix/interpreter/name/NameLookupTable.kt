    ...
    fun traverse(scope: List<String>, defs: List<BibixAst.Def>) {
      defs.forEach { def ->
        when (def) {
          ...
          is BibixAst.ArgDef -> {
            val cname = CName(context.sourceId, scope + def.name())
            definitions[cname] = Definition.ArgDef(cname, def)
          }

          is BibixAst.ArgRedef -> {
            TODO()
          }
...
sealed class Definition {
  ...
  data class ArgDef(override val cname: CName, val argDef: BibixAst.ArgDef) : Definition()
  data class ArgRedef(override val cname: CName, val argRedef: BibixAst.ArgRedef) : Definition()
  ...
  // data class NativeBuildRule(override val cname:CName, val nativeBuildRule: NativeBuildRule)
}
